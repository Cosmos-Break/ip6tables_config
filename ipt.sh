...
rmDnat(){
    ...
    echo -n "输入 删除PREROUTING编号:";read -a pre_nums
    echo -n "输入 删除POSTROUTING编号:";read -a post_nums
...
select todo in 增加转发规则 删除转发规则 列出所有转发规则 删除所有转发规则 查看当前ip6tables配置
do
    case $todo in
    增加转发规则)
        dnat
